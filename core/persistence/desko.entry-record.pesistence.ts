...
export default class DeskoEntryRecordPersistence {
  public async save(eventControlId) {
    ...
    await Database.table('entry_records').insert({name: eventTypeIdToText(eventControlId), created_at: now, updated_at: now})
  }

  public async lastPassRecord(eventControlId): Promise<any> {
    return  this.query().where('name', eventTypeIdToText(eventControlId)).orderBy('created_at', 'desc').first()
  }
  ...
  public async getDatetimeLastRecord(eventControlId): Promise<string> {
    ...
    const lastRecord = await this.query().where('name', eventTypeIdToText(eventControlId)).orderBy('created_at', 'desc').first()
