  ...
  public async getBooking(uuid): Promise<object | boolean> {
    ...
    if (emailsOnHomologation && Array.isArray(emailsOnHomologation)) {
      ...
      if (emailsOnHomologation?.includes(booking?.person?.email?.toLowerCase())) {
        console.warn(
          `Email ${booking?.person?.email?.toLowerCase()} ESTÁ dentro da homologação - Controle de acesso ATIVADO`
        )
        return booking
      }
      console.warn(
        `Email ${booking?.person?.email?.toLowerCase()} NÃO ESTÁ dentro da homologação - Controle de acesso DESATIVADO`
      )
      return false
    }
    return booking
